...
void Model::render(glm::vec3 pos, float ang, float scale, GLint matLoc, glm::mat4 modelMat, GLint colorLoc) {
  int numTriangles = -1;
  glm::mat4 model{1.0f};
  ...
  // ----------------------
  glUniformMatrix4fv(matLoc, 1, GL_FALSE, &modelMat[0][0]);

  glUniformMatrix4fv(matLoc, 1, GL_FALSE, &model[0][0]);
  // auto modelViewMatrix{glm::mat3(viewCamera * m_modelMatrix)};
  // glm::mat3 normalMatrix{glm::inverseTranspose(modelViewMatrix)};
  // glUniformMatrix3fv(norLoc, 1, GL_FALSE, &normalMatrix[0][0]);
  // ----------------------
  GLsizei numIndices = (numTriangles < 0) ? m_indices.size() : numTriangles * 3;
  ...
  if(pos.z > mirrorPz){
    glFrontFace(GL_CW);
    pos.z = 2*mirrorPz-pos.z;

    model = glm::mat4(1.0f);
    model = glm::translate(model, pos);
    model = glm::scale(model, glm::vec3(scale) * glm::vec3(1.0f, 1.0f, -1.0f));
    model = glm::rotate(model, ang, glm::vec3(0, 1, 0));

    glUniformMatrix4fv(matLoc, 1, GL_FALSE, &model[0][0]);
    glUniform4f(colorLoc, 1.0f, 1.0f, 1.0f, 0.3f);
    glDrawElements(GL_TRIANGLES, numIndices, GL_UNSIGNED_INT, nullptr);
    glFrontFace(GL_CCW);
  }
...
void Model::setMirrorPz(float pz){
  mirrorPz = pz;
}